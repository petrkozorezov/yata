...
- **Интерфейсы**: gRPC-сервер отвечает за преобразование protobuf-сообщений во внутренние команды и возврат унифицированных ответов. Внешние callback-и платежного сервиса попадают в отдельный эндпойнт, но используют те же хендлеры. Появление REST/GraphQL интерфейса не потребует переписывать бизнес-логику.
...
- Настроить окружения (dev/prod), чтобы легко переключаться между ETS и Ecto-адаптерами и автоматизировать `ecto`-таски.
- Расширить тесты негативными сценариями и добавить контрактные проверки gRPC.
- Подготовить read-model API для получения статусов заказов и платежей.
